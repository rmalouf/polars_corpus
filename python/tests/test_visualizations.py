...
def test_barcode_plot_rows_key_off_the_term_column() -> None:
    ...
    ax = barcode_plot(CORPUS.rename({"token": "word"}), "word", ["the", "cat"])
    assert y_labels(ax) == ["the", "cat"]
...
@pytest.mark.parametrize("reverse,alignment", [(False, "left"), (True, "right")])
def test_keyword_plot_reverse_mirrors_the_chart(reverse: bool, alignment: str) -> None:
    # Mirrored, the stems run leftwards and the labels follow them, for putting
    # two keyword plots back to back.
    ax = keyword_plot(KEYWORDS, "token", "LL", reverse=reverse)
    left, right = ax.get_xlim()
    assert (left > right) == reverse
    assert {text.get_ha() for text in ax.texts} == {alignment}
