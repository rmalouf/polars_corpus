...
#[polars_expr(output_type_func=t_test_output_schema)]
fn welchs_t_from_stats(inputs: &[Series], kwargs: AlternativeKwargs) -> PolarsResult<Series> {
    let m1: Option<f64> = inputs[0].f64()?.get(0);
    let v1: Option<f64> = inputs[1].f64()?.get(0);
    let n1: Option<f64> = inputs[2].cast(&DataType::Float64)?.f64()?.get(0);

    let m2: Option<f64> = inputs[3].f64()?.get(0);
    let v2: Option<f64> = inputs[4].f64()?.get(0);
    let n2: Option<f64> = inputs[5].cast(&DataType::Float64)?.f64()?.get(0);

    let (t, p, df): (Option<f64>, Option<f64>, Option<f64>) = match (m1, m2, v1, v2, n1, n2) {
        (Some(m1), Some(m2), Some(v1), Some(v2), Some(n1), Some(n2)) if v1 > 0.0 && v2 > 0.0 => {
            let vn1 = v1 / n1;
            let vn2 = v2 / n2;
            let t = (m1 - m2) / (vn1 + vn2).sqrt();
            let df = (vn1 + vn2).powi(2) / (vn1.powi(2) / (n1 - 1.) + (vn2.powi(2) / (n2 - 1.)));
